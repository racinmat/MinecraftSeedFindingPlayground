...
// Create data-frame in memory
    ...
    val sum = df.summarize(
            ...
            "biome in dark forest2" to {it["biome"].asStrings().sumBy { if(it in listOf("dark_forest", "dark_forest_hills")) 1 else 0 }},
            "layer18 dark forest or ocean" to {it["layer18"].eq("dark_forest").count{ it } + it["layer18"].eq("ocean").count{ it }}
    )


    println(df["biome"].asStrings().map { it in listOf("dark_forest", "dark_forest_hills") }.map { if (it) 1 else 0 }.sum())
    println(df["biome"].asStrings().sumBy { if(it in listOf("dark_forest", "dark_forest_hills")) 1 else 0 })
    println(df)
    println(sum)
// Or from csv
